'use strict';
document.querySelector('#Menu li.Menu-item[data-work="14"]').addEventListener('click', renderTask);
document.querySelector('#Menu--tablet li.Menu-item[data-work="14"]').addEventListener('click', renderTask);
  ...
  return function (arg1, arg2) {
    ...
    if (typeof arg1 === 'number' && typeof  arg2 === 'number') {
      for (i = 0; i < cache.length; i++) {
        ...
        arg1 = cachedResult[0] === arg1;

        if (!arg1) {
          arg1 = cachedResult[0] === arg2;
          arg2 = cachedResult[1] === arg1;
        } else {
          arg2 = cachedResult[1] === arg2;
        }

        if (arg1 && arg2) return cachedResult[2];
      }
    } else {
      for (i = 0; i < cache.length; i++) {
        cachedResult = cache[i];
        if (cachedResult[0] === arg1 && cachedResult[1] === arg2) return cachedResult[2];
      }
    }

    result = func(arg1, arg2);
    cache.push([arg1, arg2, result]);
    return result;
  };
}
...
let cache = makeCache(complexFunction);
...
function getResize (e) {
  ...
  resizewidth = setTimeout(function () {
    (alert(`Document width is ${window.outerWidth} px, height is ${window.outerHeight} px;`));
  }, 2000);
}
