...
class SnackBar {
  constructor ({
    ...
    const snackbarMessage = $(document.createElement('p')).appendTo(snackbar);
    ...
    if (name && typeof(name) === 'string') {
      snackbar.data('snackbar', name);
    }

    if (confirm) {
      buttonWrapper = $(document.createElement('div')).appendTo(snackbar);
      ...
      resolveButton.on('click', this.hide);
      rejectButton.on('click', this.hide);
    }

    if (progressbar) {
      progressBar = $(document.createElement('div')).appendTo(snackbar);
  ...
  hide () {
    ...
    if (this.progressBar !== undefined) {
      this._snackbar.on('transitionend', () => {
        this.progressBar.attr('style', '');
      });
    }
  }
}
...
class StudentsView extends EventEmitter {
  ...
  static askForDelete (index) {
    let deletePopup = this._confirms.deletePopup;
    ...
    if (deletePopup === undefined) {
      ...
      deletePopup._resolveButton.on('click', () => (this.emit('deleteStudentRequest', index)));

      this._confirms.deletePopup = deletePopup;
